...
def helper(dis):
    ...
    pre = precautions.loc[precautions['Disease'] == dis, ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.flatten()
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:
            ...
            st.warning(f"Symptom '{item}' not recognized. Please check your input.")

    if valid_symptoms:
        return diseases_list[svc.predict([input_vector])[0]]
    else:
        return "No valid symptoms entered for prediction."
...
if symptoms:
    ...
    if st.button("Predict Disease"):
        ...
        display_list("Precautions", pre)
        display_list("Medications", med[0].strip("[]").replace("'","").split(', ') if len(med) > 0 else [])
        display_list("Workouts", wrkout)
        display_list("Diets", die[0].strip("[]").replace("'","").split(', ') if len(die) > 0 else [])
